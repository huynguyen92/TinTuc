...
import com.solodroid.androidnewsapp.adapter.Adapter_Favorite;
...
public class NewsFavoriteActivity extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle(R.string.menu_favorite);
		...
		adapter = new Adapter_Favorite(NewsFavoriteActivity.this,
		...
		list_fav.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1,
				...
				int pos = Integer.parseInt(pojo.getnId());

				Intent intplay = new Intent(NewsFavoriteActivity.this,
	...
	public void onResume() {
		...
		for (int j = 0; j < allData.size(); j++) {
			...
			allListNewsCatId.add(objAllBean.getnId());
			...
			allListNewsDate.add(String.valueOf(objAllBean.getNewSource()));
				...
				for (int i = 0; i < allArrayNewsHeading.length; i++) {
					if (textlength <= allArrayNewsHeading[i].length()) {
						if (newText.toString().equalsIgnoreCase(
							...
							objItem.setnId(allArrayNewsCatId[i]);
							...
							objItem.setNewSource(allArrayNewsDate[i]);
